...
1. (x + a, y + b)에서 a + b = 4일때까지 탐색하면 된다(cnt = 4).
2. 한 노드 기준으로는 위, 아래, 왼쪽, 오른쪽을 탐색하면 된다(dx, dy 배열).
3. 방문한 노드를 표시하기 위해 visited를 true로 바꾼다.
   이렇게 하지 않으면 x, y + 1에서 탐색할 때 다시 이전 노드를 탐색하게 된다.
4. 재귀가 끝나면, 다음번 탐색할 때를 위해 visited를 false로 바꾼다(백트래킹).
5. ㅗ, ㅓ, ㅜ, ㅏ 는 백트래킹으로 탐색이 불가능하므로 따로 로직을 만든다.
*/
